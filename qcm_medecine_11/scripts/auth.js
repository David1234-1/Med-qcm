// Gestionnaire d'authentification pour QCM Médecine 11
class AuthManager {
  constructor() {
    this.auth = null;
  ...
  initEvents() {
    ...
    // Boutons d'accès invité
    const guestButtons = document.querySelectorAll('.btn-guest, #guest-access-btn, #guest-access-btn-register');
  ...
  enableGuestMode() {
    ...
    // Sauvegarder l'état en mode invité
    ...
    this.showMainApp();
    this.updateUserInfo();
    this.showNotification('Mode invité activé. Vos données seront sauvegardées localement.', 'info');
  }

  checkGuestMode() {
    const guestMode = localStorage.getItem('qcm_medecine_11_guest_mode');
    const guestUser = localStorage.getItem('qcm_medecine_11_guest_user');
    ...
    if (guestMode === 'true' && guestUser) {
      try {
        ...
        console.error('Erreur lors du chargement du mode invité:', error);
        localStorage.removeItem('qcm_medecine_11_guest_mode');
        localStorage.removeItem('qcm_medecine_11_guest_user');
      }
    }
  }
  ...
  async logout() {
    try {
      if (this.isGuestMode) {
        // Mode invité - juste nettoyer les données locales
        this.currentUser = null;
        this.isGuestMode = false;
        localStorage.removeItem('qcm_medecine_11_guest_mode');
        localStorage.removeItem('qcm_medecine_11_guest_user');
        this.showAuthOverlay();
        this.showNotification('Mode invité désactivé', 'info');
      } else {
        // Utilisateur connecté - déconnexion Firebase
  ...
  handleAuthStateChange(user) {
    if (user) {
      ...
      // Utilisateur déconnecté - vérifier le mode invité
      if (!this.isGuestMode) {
        this.currentUser = null;
        this.showAuthOverlay();
      }
  ...
  handleAuthError(error) {
    ...
    switch (error.code) {
      ...
      case 'auth/api-key-not-valid':
        message = 'Erreur de configuration. Utilisez le mode invité.';
        break;
      default:
